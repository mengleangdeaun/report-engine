...
import RequestTopUpModal from '../../../components/Report/RequestTopUpModal'; // Import Modal
...
import {
  ...
  IconFile, IconFileSearch, IconChevronDown
...
const TiktokReportGenerator = () => {
  ...
  /* ── Page names autocomplete ── */
  const fetchPageNames = async () => {
    setPagesLoading(true);
    try {
      const res = await api.get('/user/page-names', { params: { platform: 'tiktok' } });
      setPageOptions((res.data || []).map((p: any) => ({ label: p.name, value: p.name, id: p.id, creator: p.creator_name })));
    } catch { }
  ...
  const handleMediaSelect = (mediaFile: any) => {
    ...
    setShowMediaSelector(false); // Close the modal
  };
  ...
  const handleFileChange = (f: File | null) => {
    ...
    setFile(f);
    setSelectedMediaFile(null); // Clear selected media file
  };

  /* ── Generate report ── */
  const handleGenerate = async () => {
    if (!file && !selectedMediaFile) { toast.error('Please select or upload a CSV file.'); return; }
    if (!pageName) { toast.error('Please enter a Tiktok Profile name.'); return; }
    ...
    const apiPromise = api.post('/generate-report', fd, {
  ...
  return (
    <div className="space">
              ...
              <div className="space-y-12">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <CreatableSelect
                      ...
                      formatOptionLabel={(option: any) => (
                        <div className="flex items-center justify-between">
                          ...
                          {option.creator && (
                            <span className="text-[10px] font-semibold uppercase tracking-wider bg-gray-200 dark:bg-gray-600 px-2 py-0.5 rounded-full text-gray-600 dark:text-gray-200">
                    ...
                    ${isFocused
                            ? '!border-gray-900 dark:!border-gray-400 !ring-2 !ring-gray-900/20 dark:!ring-gray-400/20'
                  ...
                  `,
                    ...
                    {pageName && (
                      <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700 transition-all">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-100 dark:to-gray-200 flex items-center justify-center flex-shrink-0 shadow-sm">
                            <IconBrandTiktok size={18} className="text-white dark:text-gray-900" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{pageName.label}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">TikTok Profile · Ready for analysis</p>
                          </div>
                          <button
                            onClick={() => setPageName(null)}
                            className="w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-red-100 dark:hover:bg-red-900/30 flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors flex-shrink-0"
                          >
                ...
                <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-2">
                        {['CSV', 'XLSX', 'XLS'].map((fmt) => (
                          <span key={fmt} className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700">
                            {fmt}
                          </span>
                        ))}
                      </div>
                  ...
                  <motion.div
                    key={resetKey}
                    className={`relative border-2 ${isDragging ? 'border-rose-500 bg-rose-50/30 dark:bg-rose-900/10' : 'border-dashed border-gray-300 dark:border-gray-600'} rounded-xl p-8 text-center transition-all duration-200`}
                    ...
                    <div className="max-w-md mx-auto space-y-5">
                      ...
                      {file ? (
                        <div className="p-5 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 transition-all">
                          ...
                          <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => handleFileChange(e.target.files?.[0] ?? null)} accept=".csv,.xlsx,.xls" id="tiktok-file-upload" />
                        </div>
                      ) : selectedMediaFile ? (
                        ...
                        <div
                          className="relative block w-full p-6 bg-gray-50 dark:bg-gray-900/50 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 cursor-pointer hover:border-gray-900 dark:hover:border-gray-400 transition-colors group"
                          onClick={() => fileInputRef.current?.click()}
                        >
                          <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => handleFileChange(e.target.files?.[0] ?? null)} accept=".csv,.xlsx,.xls" id="tiktok-file-upload" />
                          <div className="w-12 h-12 bg-rose-100 dark:bg-rose-900/40 rounded-xl flex items-center justify-center mb-3 mx-auto group-hover:scale-110 transition-transform">
                            <IconUpload size={24} className="text-rose-500" />
                          </div>
                          <p className="font-medium text-gray-700 dark:text-gray-300 text-sm">Drop your file here or click to browse</p>
                      ...
                      >
                        <div className={`flex items-center gap-1 ${col.align === 'right' ? 'justify-end' : col.align === 'center' ? 'justify-center' : 'justify-start'
                          ...
                          <span className={`flex flex-col leading-none text-[8px] ${sortKey === col.key ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'}`}>
                            <IconChevronUp size={12} className={sortKey === col.key && sortDir === 'asc' ? 'text-rose-600' : 'text-gray-400'} />
                            <IconChevronDown size={12} className={`-mt-1 ${sortKey === col.key && sortDir === 'desc' ? 'text-rose-600' : 'text-gray-400'}`} />
                          </span>
                        </div>
                      </th>
                    ))}
      ...
      />
      <ProcessingOverlay isOpen={loading} text="Analyzing TikTok Data..." />
