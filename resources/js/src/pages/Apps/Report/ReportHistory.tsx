...
const ReportHistory = () => {
    ...
    // --- DATE FILTER STATE (Single State for Range) ---
    const [dateRange, setDateRange] = useState<any>(null); // [Date, Date]
...
const columns = useMemo(() => [
    ...
    columnHelper.accessor('platform', {
        ...
        cell: info => {
            ...
            return (
                <span className={`badge flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-md border ${
                    isFb 
...
<div className="mb-8 space-y-6">
  ...
  <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
    ...
    {/* Filter Grid - All filters in one cohesive layout */}
    ...
    <div className="relative">
                ...
                <Listbox.Options className="absolute z-50 mt-1 w-full rounded-lg bg-white dark:bg-gray-800 py-2 shadow-lg border border-gray-200 dark:border-gray-700 focus:outline-none max-h-64 overflow-auto">
                  <div className="px-3 py-2 border-b border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-gray-500">Select members</span>
                      {selectedUsers.length > 0 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
      ...
      <div>
        ...
        <div className="relative">
          <Flatpickr
            ...
            className="w-full pl-10 pr-4 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800/50 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer shadow-sm"
            placeholder="Select date range"
            value={dateRange}
            onChange={(date) => { setDateRange(date); setPagination(p => ({...p, pageIndex: 0})); }}
          />
          <IconCalendar className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <input
            type="text"
            placeholder="Search reports by name, page, or ID..."
        ...
        topPerformers.map((report) => (
          <div key={report.id} className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-all duration-200 group">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className={`px-3 py-1.5 rounded-full flex items-center gap-1.5 text-xs font-semibold ${
                report.growth_rate >= 50 
                ...
                >
                    <Dialog.Panel className="relative w-full max-w-4xl transform transition-all">
                        <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
                            ...
                            <div className={`relative px-6 py-6 text-white overflow-hidden ${
                                selectedReport?.platform === 'facebook' 
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-sm border transition-all duration-300 ${
                                                    selectedReport?.platform === 'facebook' 
                                                    ...
                                                    <span className={`h-2 w-2 rounded-full me-1 ${
                                                        selectedReport?.platform === 'facebook' ? 'bg-blue-600' :
