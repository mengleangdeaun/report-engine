...
const Header = () => {
    ...
    const getProfileImage = (userObj: any) => {
        ...
        // 2. Social Login (starts with http or https)
        if (userObj.avatar.startsWith('http')) {
            return userObj.avatar;
        }

        // 3. Local Upload (Needs Backend URL prepended)
        // Adjust 'http://localhost:8000' to match your actual backend URL or env variable
        return `http://localhost:8000/storage/${userObj.avatar}`;
    };
    ...
    const [userData, setUserData] = useState(() => {
        ...
        if (storedUser) {
            try {
                ...
                // Calculate Avatar immediately using the helper
                initialData.avatar = getProfileImage(parsed);
            } catch (e) {
    ...
    // --- 2. EFFECT: UPDATE ON NAVIGATION ---
    // This ensures if you logout/login or update profile, the header updates
    useEffect(() => {
        const storedUser = localStorage.getItem('user') || sessionStorage.getItem('user');
        if (storedUser) {
            try {
                        ...
                        <div className="dropdown shrink-0 flex">
                            ...
                            >
                                <ul className="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                                    <li>
                                        <div className="flex items-center px-4 py-4">
                                            ...
                                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                                ...
                                                <button type="button" className="text-black/60 hover:text-primary dark:text-dark-light/60 dark:hover:text-white text-xs block truncate w-full text-left">
                                                    {userData.email}
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <li>
                                        <Link to="/settings/preferences" className="dark:hover:text-white">
                                            <svg className="ltr:mr-2 rtl:ml-2 shrink-0" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                                    ...
                                    <li className="border-t border-white-light dark:border-white-light/10">
                                        <button onClick={handleLogout} className="text-danger !py-3 w-full flex items-center px-4 hover:bg-gray-100 dark:hover:bg-[#1b2e4b]">
                                            <svg className="ltr:mr-2 rtl:ml-2 rotate-90 shrink-0" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path opacity="0.5" d="M17 9.00195C19.175 9.01406 20.3529 9.11051 21.1213 9.8789C22 10.7576 22 12.1718 22 15.0002V16.0002C22 18.8286 22 20.2429 21.1215C20.2426 22.0002 18.8284 22.0002 16 22.0002H8C5.17157 22.0002 3.75736 22.0002 2.87868 21.1215C2 20.2429 2 18.8286 2 16.0002L2 15.0002C2 12.1718 2 10.7576 2.87868 9.87889C3.64706 9.11051 4.82497 9.01406 7 9.00195" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
